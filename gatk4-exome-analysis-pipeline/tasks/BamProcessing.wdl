...
task SortSam {
  ...
  command {
    java -Dsamjdk.compression_level=~{compression_level} -Xms4000m -Xmx4000m -jar /usr/picard/picard.jar \
      SortSam \
      INPUT=~{input_bam} \
...
task MarkDuplicates {
  ...
  command {
    java -Dsamjdk.compression_level=~{compression_level} -Xmx~{java_memory_size}g -jar /usr/picard/picard.jar \
      MarkDuplicates \
...
task ApplyBQSR {
  ...
  command {
    gatk --java-options "-XX:+PrintFlagsFinal -XX:+PrintGCTimeStamps -XX:+PrintGCDateStamps \
      ...
      -XX:GCTimeLimit=50 -XX:GCHeapFreeLimit=10 -Dsamjdk.compression_level=~{compression_level} -Xmx3000m" \
      ApplyBQSR \
...
task GatherSortedBamFiles {
  ...
  command {
    java -Dsamjdk.compression_level=~{compression_level} -Xmx2000m -jar /usr/picard/picard.jar \
      GatherBamFiles \
      INPUT=~{sep=' INPUT=' input_bams} \
      OUTPUT=~{output_bam_basename}.bam \
      CREATE_INDEX=true \
